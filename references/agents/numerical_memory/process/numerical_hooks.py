...
from hook_utils import prepare_field_context
...
# _get_row6_fields 已废弃，统一使用 prepare_field_context
def _get_row6_fields(table_name):
    """兼容旧调用：返回 Row6 英文字段名列表"""
    ctx = prepare_field_context([table_name])
    fm = ctx['field_maps'].get(table_name, {})
    return fm.get('en', [])
...
def on_enter_output():
    ...
    table_names = list(filled['tables'].keys()) if filled and 'tables' in filled else []
