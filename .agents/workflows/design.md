...
1. 读取 `.env` 获取 `WORKSPACE_DIR`，确认项目根目录路径。

2. 运行初始化脚本（路径相对于 `references/`，即本仓库根目录）：
...
3. 读取状态文件 `scripts/output/task_state.json`，确认当前状态

4. 根据当前状态加载对应 Agent 的知识库：
