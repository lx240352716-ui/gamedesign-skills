...
```shell
python scripts/core/workflow_runner.py --skill design --start-at <目标状态>
```
...
1. /quick 的案例已由 HFSM hooks 自动暂存
2. 自查：用户在 Step 4 确认时是否纠正了方案？
   - 有纠正 → 调用 `append_pending()` 写入踩坑记录 → `commit_pending()`
   - 无纠正 → 跳过
